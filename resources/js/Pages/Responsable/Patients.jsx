...
function ErrorText({ message }) {
    ...
    return <div style={{ color: 'var(--red)', fontSize: 12, fontWeight: 700, marginTop: 6 }}>{message}</div>;
}
...
const text = {
    fr: {
        ...
        cols: ['Nom','Email','Téléphone','Adresse','Naissance','Lien','État','Médecin','Adhérence','Actions'],
        ...
        retirer: 'Retirer', programme: 'Programme', etat_options: ['actif', 'inactif', 'gueri'],
    },
    en: {
        titre: 'Patient Management', nb: 'patient(s) registered',
        ajouter: '+ Add patient', fermer: '✕ Close', nouveau: 'New patient',
        cols: ['Name','Email','Phone','Address','Birth','Link','Status','Doctor','Adherence','Actions'],
        ...
        retirer: 'Remove', programme: 'Program', etat_options: ['actif', 'inactif', 'gueri'],
    },
    ar: {
        titre: 'إدارة المرضى', nb: 'مريض مسجل',
        ajouter: '+ إضافة مريض', fermer: '✕ إغلاق', nouveau: 'مريض جديد',
        cols: ['الاسم','البريد','الهاتف','العنوان','الميلاد','الصلة','الحالة','الطبيب','الالتزام','الإجراءات'],
        ...
        retirer: 'إزالة', programme: 'برنامج اليوم', etat_options: ['actif', 'inactif', 'gueri'],
    },
};
...
export default function Patients() {
    ...
    const addForm  = useForm({ nom: '', prenom: '', email: '', telephone: '', adresse: '', password: '', password_confirmation: '', lien: 'autre', date_naissance: '', medecin_id: '' });
    const editForm = useForm({ nom: '', prenom: '', email: '', telephone: '', adresse: '', lien: 'autre', etat: 'actif', date_naissance: '', medecin_id: '' });

    const openEdit = (p) => {
        setEditPatient(p);
        setShowAdd(false);
        editForm.setData({ nom: p.nom, prenom: p.prenom, email: p.email, telephone: p.telephone, adresse: p.adresse || '', lien: p.lien, etat: p.etat, date_naissance: p.date_naissance || '', medecin_id: p.medecin_id || '' });
        window.scrollTo(0, 0);
    };

    const handleAdd    = () => {
        addForm.post('/responsable/patients', {
            preserveScroll: true,
    ...
    const handleProgramme = (patient) => router.post(`/responsable/patients/${patient.id}/programme`, {}, { preserveScroll: true });
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16 }}>
                        {[[t.nom,'nom'],[t.prenom,'prenom'],[t.email,'email'],[t.telephone,'telephone']].map(([l,k]) => (
                            <div key={k}>
                                ...
                                <input style={inp(addForm.errors[k])} value={addForm.data[k]} onChange={e => addForm.setData(k, e.target.value)} onFocus={onFocus} onBlur={onBlur} />
                                <ErrorText message={addForm.errors[k]} />
                            </div>
                        ))}
                        <div>
                            <label style={lbl}>{t.password}</label>
                            <input type="password" style={inp(addForm.errors.password)} value={addForm.data.password} onChange={e => addForm.setData('password', e.target.value)} onFocus={onFocus} onBlur={onBlur} />
                            <ErrorText message={addForm.errors.password} />
                        </div>
                        <div>
                            <label style={lbl}>{t.password_confirm}</label>
                            <input type="password" style={inp(addForm.errors.password_confirmation)} value={addForm.data.password_confirmation} onChange={e => addForm.setData('password_confirmation', e.target.value)} onFocus={onFocus} onBlur={onBlur} />
                            <ErrorText message={addForm.errors.password_confirmation} />
                        </div>
                        <div>
                            <label style={lbl}>{t.adresse}</label>
                            <input style={inp(addForm.errors.adresse)} value={addForm.data.adresse} onChange={e => addForm.setData('adresse', e.target.value)} onFocus={onFocus} onBlur={onBlur} />
                            <ErrorText message={addForm.errors.adresse} />
                        </div>
                        <div>
                            <label style={lbl}>{t.lien}</label>
                            <select style={inp(addForm.errors.lien)} value={addForm.data.lien} onChange={e => addForm.setData('lien', e.target.value)} onFocus={onFocus} onBlur={onBlur}>
                                {LIENS.map(l => <option key={l}>{l}</option>)}
                            </select>
                            <ErrorText message={addForm.errors.lien} />
                        </div>
                        <div>
                            <label style={lbl}>{t.date_naissance}</label>
                            <input type="date" style={inp(addForm.errors.date_naissance)} value={addForm.data.date_naissance} onChange={e => addForm.setData('date_naissance', e.target.value)} onFocus={onFocus} onBlur={onBlur} />
                            <ErrorText message={addForm.errors.date_naissance} />
                        </div>
                        <div>
                            <label style={lbl}>{t.medecin}</label>
                            <select style={inp(addForm.errors.medecin_id)} value={addForm.data.medecin_id} onChange={e => addForm.setData('medecin_id', e.target.value)} onFocus={onFocus} onBlur={onBlur}>
                                <option value="">{t.aucun_medecin}</option>
                                {medecins.map(m => <option key={m.id} value={m.id}>Dr. {m.nom} {m.prenom}</option>)}
                            </select>
                            <ErrorText message={addForm.errors.medecin_id} />
                        </div>
                    </div>
                    <div style={{ display: 'flex', gap: 10, marginTop: 20 }}>
                        <button disabled={addForm.processing} onClick={handleAdd} style={{ background: 'var(--blue)', color: '#fff', border: 'none', borderRadius: 8, padding: '10px 20px', cursor: addForm.processing ? 'not-allowed' : 'pointer', opacity: addForm.processing ? .7 : 1, fontWeight: 800 }}>{t.enregistrer}</button>
                ...
                <div style={{ ...cardStyle, padding: 20, marginBottom: 16, borderLeft: '3px solid var(--amber)' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16 }}>
                        {[[t.nom,'nom'],[t.prenom,'prenom'],[t.email,'email'],[t.telephone,'telephone']].map(([l,k]) => (
                            <div key={k}>
                                <label style={lbl}>{l}</label>
                                <input style={inp(editForm.errors[k])} value={editForm.data[k]} onChange={e => editForm.setData(k, e.target.value)} onFocus={onFocus} onBlur={onBlur} />
                                <ErrorText message={editForm.errors[k]} />
                            </div>
                        ))}
                        <div>
                            <label style={lbl}>{t.adresse}</label>
                            <input style={inp(editForm.errors.adresse)} value={editForm.data.adresse} onChange={e => editForm.setData('adresse', e.target.value)} onFocus={onFocus} onBlur={onBlur} />
                            <ErrorText message={editForm.errors.adresse} />
                        </div>
                        <div>
                            <label style={lbl}>{t.lien}</label>
                            <select style={inp(editForm.errors.lien)} value={editForm.data.lien} onChange={e => editForm.setData('lien', e.target.value)} onFocus={onFocus} onBlur={onBlur}>
                                {LIENS.map(l => <option key={l}>{l}</option>)}
                            </select>
                            <ErrorText message={editForm.errors.lien} />
                        </div>
                        <div>
                            <label style={lbl}>{t.date_naissance}</label>
                            <input type="date" style={inp(editForm.errors.date_naissance)} value={editForm.data.date_naissance} onChange={e => editForm.setData('date_naissance', e.target.value)} onFocus={onFocus} onBlur={onBlur} />
                            <ErrorText message={editForm.errors.date_naissance} />
                        </div>
                        <div>
                    ...
                    <div style={{ display: 'flex', gap: 10, marginTop: 20 }}>
                        <button disabled={editForm.processing} onClick={handleEdit} style={{ background: 'var(--amber)', color: '#fff', border: 'none', borderRadius: 8, padding: '10px 20px', cursor: editForm.processing ? 'not-allowed' : 'pointer', opacity: editForm.processing ? .7 : 1, fontWeight: 800 }}>{t.mettre_a_jour}</button>
                    ...
                    <tbody>
                        {filtered.map(p => (
                            ...
                            >
                                ...
                                <td style={{ ...td, color: 'var(--muted)' }}>{p.adresse || '-'}</td>
                                <td style={{ ...td, color: 'var(--muted)' }}>{p.date_naissance || '-'}</td>
                                <td style={td}>
                                ...
                                <td style={td}>
                                    ...
                                    <button onClick={() => handleProgramme(p)} title={t.programme} style={{ background: 'rgba(99,102,241,0.1)', color: 'var(--blue)', border: 'none', borderRadius: 8, cursor: 'pointer', marginRight: 6, fontSize: 12, fontWeight: 800, padding: '5px 8px' }}>{t.programme}</button>
